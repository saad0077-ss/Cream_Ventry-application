...
class _ScreenSignInState extends State<ScreenSignIn>
  ...
  Widget build(BuildContext context) {
    ...
    final bool isMediumScreen =screenHeight >601 && screenHeight < 1010;  
 
    final bool isSmallScreen = screenHeight <= 599;

    // Mobile/Tablet layout 
    // final double screenHeight = MediaQuery.of(context).size.height;v
    final double horizontalPadding = 22.5;

    // Responsive container height  
    final double containerHeight = isMediumScreen ? 500.0 : isSmallScreen ? 460.0 : 590.0;

    // Calculate container width based on screen width
    final double containerWidth; 
    if (screenWidth >= 506) {   
      // Between 546 and 1000: fixed width of 500px (546 - 2*22.5 pad ding)
      containerWidth = 430.0; 
    } else {
