...
class _PaymentInTransactionState extends State<PaymentInTransaction> {
  ...
  void initState() {
    super.initState();
    // Ensure PaymentInDb is initialized
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: Container(
        ...
        child: Stack(
          children: [
            ValueListenableBuilder<List<PaymentInModel>>(
              ...
              builder: (context, allPayments, _) {
                ...
                final totalPayment = filteredPayments.fold<double>(
                    0.0, (sum, payment) => sum + payment.receivedAmount);
                debugPrint('Filtered payments: ${filteredPayments.length}, Total: $totalPayment');
